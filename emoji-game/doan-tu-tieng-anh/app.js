...
function start(){state.deck=buildDeck();state.i=0;state.score=0;state.correct=0;state.streak=0;show("gameScreen");render()}function render(){const q=state.deck[state.i];$("#roundText").textContent=`Câu ${state.i+1}/${state.deck.length}`;$("#scoreText").textContent=state.score;$("#progressBar").style.width=`${((state.i+1)/state.deck.length)*100}%`;$("#wordClue").textContent=q.emoji;$("#questionText").textContent=`Từ nào có nghĩa là “${q.vi}”?`;$("#hintText").textContent="Chọn từ tiếng Anh đúng nhất.";$("#answerGrid").innerHTML=shuffle(q.c).map(c=>`<button class="answer-button" data-a="${c}">${c}</button>`).join("");$$(".answer-button").forEach(b=>b.onclick=()=>answer(b,q))}
function answer(btn,q){const ok=btn.dataset.a===q.a;$$(".answer-button").forEach(b=>{b.disabled=true;if(b.dataset.a===q.a)b.classList.add("correct")});if(!ok)btn.classList.add("wrong");if(ok){state.score+=100+state.streak*25;state.correct++;state.streak++;$("#feedbackIcon").textContent="✅";$("#feedbackLabel").textContent="Correct";}else{state.streak=0;$("#feedbackIcon").textContent="📘";$("#feedbackLabel").textContent="Not quite"}$("#feedbackTitle").textContent=q.a;$("#feedbackText").textContent=q.ex;state.i++;setTimeout(()=>show("feedbackScreen"),400)}
